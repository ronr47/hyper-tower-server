...
#![allow(dead_code)] // Suppress intermediate structural hooks warnings
...
impl AsyncMetricsDaemon {
    ...
    pub async fn run_metrics_watcher_loop(&mut self) -> Result<(), &'static str> {
        loop {
            ...
            // Isolated block scope guarantees raw pointers drop before an await checkpoint
            {
            ...
            // Execution yielded back cleanly to Tokio core executor
